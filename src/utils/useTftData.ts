  ...
  const { data, isLoading, refetch, error: fetchError } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Check if this is a group/continent or 'all'
        if (currentRegion === 'all') {
          // Get all individual regions (not groups)
          const allRegions = REGIONS.filter(r => !r.isGroup && r.id !== 'all').map(r => r.id);
          responseData = await mergeRegionData(allRegions);
        } else if (isRegionGroup(currentRegion)) {
        ...
        if (!items || items.length === 0) {
          ...
          const extractedItems = responseData?.compositions 
            ? extractItemsFromCompositions(responseData.compositions)
            : [];
